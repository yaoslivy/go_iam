...
import (
	"context"
	"fmt"
	"go_iam/internal/pump/analytics"
	"go_iam/internal/pump/config"
	"go_iam/internal/pump/options"
	"go_iam/internal/pump/pumps"
	"go_iam/internal/pump/storage"
	"go_iam/internal/pump/storage/redis"
	"go_iam/pkg/log"
	"sync"
	"time"
...
func (s *pumpServer) initialize() {
	...
	for key, pmp := range s.pumps {
		...
		if err != nil {
			...
			pmpIns := pmpType.New()
			initErr := pmpIns.Init(pmp.Meta) // initialize csv, mongo...
			if initErr != nil {
